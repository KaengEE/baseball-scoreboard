...
// --- 1. 개별 선수 셀 컴포넌트 ---
function SortablePlayerRow({ id, playerObj, index, onRemove, onRoleChange, isCurrentPitcher, isCurrentBatter }) {
...
<div ref={setNodeRef} style={style} className={`player-cell ${isDragging ? 'dragging' : ''}`} {...attributes} {...listeners}>
      ...
      <div className="cell-content">
        <span className={`player-name-text ${isCurrentPitcher ? 'active-p' : ''} ${isCurrentBatter ? 'active-b' : ''}`}>
          {playerObj.name}
        </span>
        
        <div className="role-radio-group" onPointerDown={(e) => e.stopPropagation()}>
          <label className={`radio-label ${isCurrentPitcher ? 'selected' : ''}`}>
            <input 
              type="radio" 
              name={`pitcher-group-${id}`} 
              checked={!!isCurrentPitcher}
              onClick={() => onRoleChange('pitcher', id)} // 토글을 위해 onClick 사용
              onChange={() => {}} // 에러 방지용 빈 함수
            /> 투수
          </label>
...
function Players({ game, allTeams, updateDB }) {
  ...
  useEffect(() => {
    if (game) {
      setTeamInputs({ awayName: game.awayName || "", homeName: game.homeName || "" });
    }
  ...
  const renderTeamBox = (team, type) => {
    return (
      <div className="team-manage-box">
        {/* 상단 팀 설정 및 로고 선택 영역 (생략, 기존 코드 유지) */}
        {team && (
          <>
                ...
                <div className="player-table-container">
                  {team.players?.map((player, index) => (
                    <SortablePlayerRow 
                      ...
                      onRoleChange={(role, pid) => handleRoleChange(team.id, role, pid)}
                    />
                  ))}
                </div>
              </SortableContext>
